...
from components.label_utils import load_qid_label_map, resolve_label
...
QID_LABELS_PATH = DATA_DIR / "wikidata_qid_labels.csv"
...
def load_decades() -> pd.DataFrame:
    ...
    df = df[(df["decade"] >= 1900) & (df["decade"] <= 2100)].copy()
...
def load_languages() -> pd.DataFrame:
    ...
    label_map = load_qid_label_map(str(QID_LABELS_PATH))
...
def load_ethnicity() -> pd.DataFrame:
    ...
    df["count"] = pd.to_numeric(df["count"], errors="coerce").fillna(0).astype(int)
    df["genderCategory"] = df["genderCategory"].astype(str).str.strip()
    df["country"] = df["country"].astype(str)

    if "ethnicGroupLabel" not in df.columns:
        df["ethnicGroupLabel"] = ""

    df["ethnicGroupLabel"] = df["ethnicGroupLabel"].fillna("").astype(str).str.strip()

    if "ethnicGroup" in df.columns:
        df["ethnicGroup"] = df["ethnicGroup"].fillna("").astype(str).str.strip()
    else:
        df["ethnicGroup"] = ""

    blank_mask = df["ethnicGroupLabel"].eq("")
    df.loc[blank_mask, "ethnicGroupLabel"] = (
        df.loc[blank_mask, "ethnicGroup"]
        .str.rsplit("/", n=1)
        .str[-1]
        .str.strip()
    )
    ...
    label_map = load_qid_label_map(str(QID_LABELS_PATH))
    df["ethnicGroupLabel"] = df["ethnicGroupLabel"].apply(
        lambda x: resolve_label(x, label_map, "Unknown ethnic group")
    )
...
def cached_gender_donut(qid: str, male: int, female: int, nb: int) -> go.Figure:
    fig = donut_gender_breakdown(male, female, nb)
    fig.update_traces(
        marker = dict(colors=["#2B6CB0", "#D53F8C", "#805AD5"])
    )
    return fig
...
def section_gender_languages_age(
    ...
    with col1:
        with card():
            ...
            if male + female + nb == 0:
                st.info("No gender breakdown data available for this country.")
            else:
                ...
                st.plotly_chart(
                    ...
                    key=f"gender_breakdown_{qid}",
                )
                st.markdown(
                    f'<div class="muted">{category_total} • Gap: {gap_pp_total:.1f} pp</div>',
                    unsafe_allow_html=True,
                )
    ...
    with col2:
        with card():
            ...
            df_lang = get_country_languages(qid)
            df_lang_view = df_lang[df_lang["type"] == lang_type].copy() if not df_lang.empty else pd.DataFrame()

            if df_lang_view.empty or df_lang_view["count"].sum() == 0:
                st.info(f"No {lang_type} language data available for this country.")
            else:
                ...
                st.plotly_chart(
                    ...
                    use_container_width=True,
                    config={"displayModeBar": False},
                    key=f"languages_{qid}_{lang_type}",
                )
            ...
            if st.checkbox("Show language details", value=False, key="dash_show_lang_details"):
                ...
                if tbl.empty:
                    st.info("No language details available.")
                else:
    ...
    with col3:
        with card():
            ...
            if df_age.empty or df_age["count"].sum() == 0:
                st.info("No age data available for this country.")
            else:
                ...
                st.plotly_chart(
                    ...
                    use_container_width=True,
                    config={"displayModeBar": False},
                    key=f"age_{qid}_{age_mode}",
                )

            if st.checkbox("Show age details", value=False, key="dash_show_age_details"):
                if df_age.empty or df_age["count"].sum() == 0:
                    st.info("No age details available.")
                else:
                    _, base_age = cached_age_figure(qid, age_mode, country_label)
...
def section_ethnicity(qid: str, country_label: str):
    ...
    for i, (g, fig, n, legend_df) in enumerate(eth_items[:4]):
        with cols[i]:
            with card():
                st.caption(f"{g} (n = {n:,})")
                st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False}, key=f"ethnicity_{qid}_{g}",)
...
def section_occupations(qid: str, country_label: str):
    ...
    df_occ = get_country_occupations(qid).copy()

    if gender_filter != "All" and not df_occ.empty and "genderCategory" in df_occ.columns:
        df_occ = df_occ[df_occ["genderCategory"].astype(str).str.strip() == gender_filter].copy()

    with card():
        if df_occ.empty or pd.to_numeric(df_occ.get("count", 0), errors="coerce").fillna(0).sum() == 0:
            ...
            st.plotly_chart(
                ...
                use_container_width=True,
                config={"displayModeBar": False},
                key=f"occupation_{qid}_{gender_filter}",
            )

    if st.checkbox("Show occupation details", value=False, key="dash_show_occ_details"):
        occ_tbl = make_occupation_details_table(get_country_occupations(qid), gender_filter=gender_filter)
        if occ_tbl.empty:
            st.info("No occupation details available.")
        else:
...
if st.button("← Back to globe"):
    st.switch_page("0_Globe_Overview.py")
